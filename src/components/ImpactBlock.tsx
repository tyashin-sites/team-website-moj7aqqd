...
function AnimatedCounter({ value, suffix, isVisible }: AnimatedCounterProps) {
  // Shared counter: 1.4s ease-out per DESIGN-SPEC §5, reduced-motion →
  // final value immediately. Same hook as MetricBar so the two counters on
  // the home page can never disagree (design-audit M-3).
...
export function ImpactBlock({
  ...
  // DESIGN-SPEC §5 reduced-motion: transforms off, opacity-only. The CSS
  // kill-switch can't reach framer's inline styles, so gate them here.
  const reduced = useReducedMotion();
  const fadeUp = reduced
    ? { initial: { opacity: 0 }, whileInView: { opacity: 1 } }
    : { initial: { opacity: 0, y: 30 }, whileInView: { opacity: 1, y: 0 } };
  const scaleIn = reduced
    ? { initial: { opacity: 0 }, whileInView: { opacity: 1 } }
