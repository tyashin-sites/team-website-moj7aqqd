          ...
          <ol className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {it.steps.map((s, idx) => (
              <li key={s.heading} className="card p-7 h-full">
                ...
                <h3 className="font-heading text-lg font-semibold tracking-tight mt-3 mb-2">
                  {s.heading}
                </h3>
        ...
        <section className="section pt-0">
          <div className="container-x">
            <div className="max-w-3xl mb-10">
              ...
              <h2 className="tt-2">From App Block to a GDPR-ready store</h2>
              <p className="mt-4 lead">{it.rich.journeyIntro}</p>
            </div>
            <ol className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-5">
              {it.rich.journey.map((s, idx) => (
                <li key={s.heading} className="card p-6 h-full flex flex-col">
                  <span className="inline-flex w-fit items-center gap-2 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-primary">
                    <span className="font-mono tabular-nums">{`0${idx + 1}`}</span>
                    {s.label}
                  </span>
                  <h3 className="font-heading text-base font-semibold tracking-tight mt-4 mb-2">
                    {s.heading}
                  </h3>
                  <p className="text-sm text-foreground/70 leading-relaxed">
                    {s.body}
        ...
        <section className="section pt-0">
          <div className="container-x">
            <div className="max-w-3xl mb-12">
              ...
              <h2 className="tt-2">Everything the {it.name} app does</h2>
              <p className="mt-4 lead">{it.rich.capabilitiesIntro}</p>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {it.rich.capabilities.map((g) => {
                const pillar = getPillar(g.pillar);
                const PIcon = PILLAR_ICON[g.pillar];
                return (
                  <div key={g.pillar} className="card p-7 h-full">
                    ...
                    <ul className="space-y-3">
                      {g.items.map((item) => (
                        <li key={item} className="flex items-start gap-2.5">
                          <Check
                            className="w-4 h-4 shrink-0 mt-1 text-primary"
                            strokeWidth={2}
                            aria-hidden
                          />
            ...
            {it.outcomes.map((o, idx) => {
              ...
              return (
                <div key={o.metric} className="glass-card p-8">
                  <dd
                    className={`font-mono tabular-nums text-4xl md:text-5xl font-medium ${idx === 0 ? "text-accent" : "text-paper"}`}
                  >
                    {m.value}
                  </dd>
                  <dt className="mt-2 text-muted-dark">{m.label}</dt>
                  <dd className="mt-3 text-sm text-muted-dark leading-relaxed">
                    {o.context}
                  </dd>
                </div>
              );
        ...
        <div className="container-x grid lg:grid-cols-12 gap-12 items-start">
          <div className="lg:col-span-4">
            ...
            <p className="mt-4 text-foreground/70">
              The questions {it.name} merchants ask most.
            </p>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <span>
                    ...
                    <span className="block mt-1 text-sm text-foreground/60 leading-relaxed">
                      {r.tagline}
                    </span>
                  </span>
                </Link>
          ...
          {relatedIndustries.length > 0 && (
            <div className="mt-8 flex flex-wrap items-center gap-x-2 gap-y-2 text-sm">
              <span className="inline-flex items-center gap-1.5 text-foreground/70">
                <Check className="w-4 h-4 text-primary" aria-hidden /> Popular
                for:
              </span>
